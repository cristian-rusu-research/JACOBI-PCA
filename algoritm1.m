function [positions, values, approx_error, tus, Ubar, S, diagonal] = algoritm1(S, alpha, g, p)
%% Algorithm 1, alpha needs to be explicitly given
%% Input:
...
if (length(alpha) ~= p)
    error('Alpha has to have dimension p');
end
...
for kk = g:-1:1
    ...
    for j = i_nuc+1:d
       aux = S(i_nuc, i_nuc)-S(j, j);
       if (j > p)
           scores(i_nuc, j) = (sqrt(aux^2+4*S(i_nuc, j)^2) - aux)*alpha(i_nuc);
       else
           scores(i_nuc, j) = (sqrt(aux^2+4*S(i_nuc, j)^2) - aux)*(alpha(i_nuc)-alpha(j));
       end
    end
    if (j_nuc > p)
        for i = 1:p
            aux = S(i, i)-S(j_nuc, j_nuc);
            scores(i, j_nuc) = (sqrt(aux^2+4*S(i, j_nuc)^2) - aux)*alpha(i);
        end
    else
        for i = 1:p
            aux = S(i, i)-S(j_nuc, j_nuc);
            scores(i, j_nuc) = (sqrt(aux^2+4*S(i, j_nuc)^2) - aux)*(alpha(i)-alpha(j_nuc));
        end
    end
end
