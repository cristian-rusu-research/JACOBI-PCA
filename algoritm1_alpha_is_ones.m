function [positions, values, approx_error, tus, Ubar, S, diagonal] = algoritm1_alpha_is_ones(S, g, p)
%% Algorithm 1, alpha is a vector of ones
%% Input:
...
for i = 1:p
    for j = p+1:d
        aux = S(i,i)-S(j,j);
        scores(i, j) = sqrt(aux^2+4*S(i,j)^2) - aux;
    end
end
...
for kk = g:-1:1
    ...
    for j = p+1:d
       ...
       scores(i_nuc, j) = sqrt(aux^2+4*S(i_nuc, j)^2) - aux;
    end
    for i = 1:p
        aux = S(i, i)-S(j_nuc, j_nuc);
        scores(i, j_nuc) = sqrt(aux^2+4*S(i, j_nuc)^2) - aux;
    end
end
